...
We present \textbf{NC-Conv} (Noise-Conditioned Convolution), a framework that resolves this trade-off by blending dynamic and static convolutional paths based on a \textbf{zero-parameter quality estimator}---directly transferring the per-sub-band selectivity modulation theory from our audio NC-SSM work~\cite{choi2026ncssm} to vision.
Each NC-Conv block computes $h = \sigma(q) \cdot h_{\text{dynamic}} + (1{-}\sigma(q)) \cdot h_{\text{static}}$, where $\sigma$ is driven by a signal-processing-based image quality estimate (not learned), ensuring automatic adaptation to spatially varying visibility \emph{without additional training on degraded data}.
...
On CIFAR-10, NC-Conv matches the standard CNN on clean data (\textbf{89.3\%} vs 88.8\%) while achieving \textbf{+7.0\%} average improvement under five corruption types at equal parameter count ($\sim$246K), with gains up to \textbf{+19.1\%} on impulse noise.
\end{abstract}
...
\begin{enumerate}
\setlength\itemsep{0pt}
\item \textbf{NC-Conv}: A noise-conditioned convolutional block that blends dynamic (input-dependent) and static (fixed-weight) paths using a 0-parameter quality estimator---the vision analog of audio NC-SSM's per-band SNR gating (Section~\ref{sec:ncconv}).
\item \textbf{0-Parameter Quality Estimator}: Michaelis-Menten normalization of local contrast, directly analogous to the audio SNR estimator, requiring no learned parameters and adapting automatically to any degradation type (Section~\ref{sec:quality}).
...
\item \textbf{Experimental validation}: +7.0\% average on corrupted CIFAR-10 at equal parameters; +19.1\% on impulse noise (Section~\ref{sec:experiments}).
\end{enumerate}
...
\subsection{Zero-Parameter Quality Estimator}
\label{sec:quality}

The quality estimate $q$ must be computed \emph{without learned parameters} to ensure:
(1)~domain-agnostic operation (works on any degradation type without retraining),
(2)~zero parameter overhead, and
(3)~consistency with the audio NC-SSM's 0-parameter SNR estimator.

\begin{definition}[Image Quality Score]
For a feature map $x \in \mathbb{R}^{C \times H \times W}$:
\begin{equation}
q = \frac{\bar{\sigma}_s}{\bar{\sigma}_s + K}, \quad
\bar{\sigma}_s = \frac{1}{C} \sum_c \text{std}(x_c),
\label{eq:quality}
\end{equation}
where $\text{std}(x_c)$ is the spatial standard deviation of channel $c$ (local contrast proxy) and $K = 0.5$ is the Michaelis-Menten half-saturation constant.
\end{definition}

This mirrors the audio SNR estimator:
\begin{center}
\setlength{\tabcolsep}{3pt}
\footnotesize
\begin{tabular}{ll}
\toprule
\textbf{Audio NC-SSM} & \textbf{Vision NC-Conv} \\
\midrule
Per-band energy $\to$ SNR & Spatial contrast $\to$ quality \\
$\text{SNR} / (\text{SNR} + K)$ & $\bar{\sigma}_s / (\bar{\sigma}_s + K)$ \\
Noise floor = 10th percentile & $K=0.5$ (fixed) \\
\bottomrule
\end{tabular}
\end{center}

Under degradation (darkness, fog, noise), spatial contrast decreases $\to$ $q$ decreases $\to$ $\sigma \to 0$ $\to$ static path dominates.
Under clean conditions, contrast is high $\to$ $q \to 1$ $\to$ $\sigma \to 1$ $\to$ dynamic path dominates.
This happens \textbf{automatically without any training on degraded data}.
...
\begin{equation}
\text{Spatial (per-frame)}: \text{NC-Conv (CNN)} \quad
...
\paragraph{Setup.}
We compare three models on CIFAR-10 classification under clean and five corruption types (severity 3), mirroring the audio noise evaluation in~\cite{choi2026ncssm}:
\begin{itemize}
\setlength\itemsep{0pt}
\item \textbf{Standard CNN}: depthwise-separable blocks, channels 48-96-192, 251K params.
\item \textbf{NC-Conv (clean)}: NC-Conv blocks, channels 44-88-176, 246K params. Trained on clean data only.
\item \textbf{NC-Conv (aug)}: Same architecture, trained with 50\% corruption augmentation.
\end{itemize}
All models use AdamW ($\text{lr}=10^{-3}$, wd $=0.05$), 5-epoch warmup + cosine schedule, 80 epochs, batch size 128.
...
\begin{table}[t]
\centering
\caption{\textbf{CIFAR-10 accuracy (\%) under clean and corrupted conditions.} NC-Conv uses 0-parameter quality estimator for $\sigma$ gating. All models $\sim$246K params.}
\label{tab:cifar10}
\setlength{\tabcolsep}{3pt}
\footnotesize
\begin{tabular}{@{}ll ccc@{}}
\toprule
\textbf{Condition} & \textbf{Audio analog} & \textbf{Std CNN} & \textbf{NC-Conv} & \textbf{NC-Conv} \\
 & & & \textbf{(clean)} & \textbf{(aug)} \\
\midrule
Clean & Clean & 88.8 & 88.9 & \textbf{89.3} \\
\midrule
Gaussian noise & White noise & 82.5 & 82.1 & \textbf{86.0} \\
Brightness $\downarrow$ & Factory (stat.) & 82.8 & 82.8 & \textbf{85.2} \\
Contrast $\downarrow$ & Babble noise & 67.7 & 70.2 & \textbf{71.9} \\
Fog & Broadband & 63.2 & 68.0 & \textbf{69.0} \\
Impulse noise & Impulse (n-s) & 48.2 & 44.8 & \textbf{67.3} \\
\midrule
Avg corrupted & & 68.9 & 69.6 & \textbf{75.9} \\
\midrule
\multicolumn{2}{l}{$\Delta$ vs Standard CNN} & --- & +0.7 & \textbf{+7.0} \\
\bottomrule
\end{tabular}
\end{table}
...
\begin{enumerate}
\setlength\itemsep{0pt}
\item \textbf{0-param $\sigma$ alone} (NC-Conv clean): +4.8\% on fog, +2.5\% on contrast vs Standard CNN, confirming that the quality estimator provides automatic degradation adaptation even without training on corrupted data.
\item \textbf{With corruption augmentation}: +7.0\% average, with +19.1\% on impulse noise. The $\sigma$ gate learns to strongly activate the static path under high-frequency degradation.
\item \textbf{Superset guarantee holds}: NC-Conv matches or exceeds the standard CNN on clean data (+0.5\%), confirming $\sigma=1$ recovery.
\end{enumerate}
...
\begin{table}[t]
\centering
\caption{\textbf{Ablation study.} Removing any NC component degrades robustness.}
\label{tab:ablation}
\setlength{\tabcolsep}{3pt}
\footnotesize
\begin{tabular}{@{}lcc@{}}
\toprule
\textbf{Model} & \textbf{Clean} & \textbf{Avg Corrupted} \\
\midrule
Standard CNN (no NC) & 88.8 & 68.9 \\
NC-Conv, $\sigma$ learned (v3) & 88.6 & 68.2 \\
NC-Conv, $\sigma$ 0-param (v4, clean) & 88.9 & 69.6 \\
NC-Conv, $\sigma$ 0-param (v4, aug) & \textbf{89.3} & \textbf{75.9} \\
\bottomrule
\end{tabular}
\end{table}

Table~\ref{tab:ablation} reveals a critical finding: \textbf{a learned $\sigma$ gate (v3) actually hurts} ($-0.7\%$ corrupted), because it overfits to clean training data.
The 0-parameter quality estimator is essential---it provides degradation-type-agnostic quality assessment, just as the audio NC-SSM's 0-parameter SNR estimator provides noise-type-agnostic signal quality.
This validates the design principle inherited from audio: \emph{the conditioning signal must come from signal processing, not from learned parameters}.
...
\subsection{Why Learned $\sigma$ Fails}

A learned $\sigma = \text{Sigmoid}(\text{Linear}(\text{GAP}(x)))$ optimizes for the training distribution.
On clean CIFAR-10, it learns $\sigma \approx \text{const}$---there is no gradient signal to differentiate clean from degraded inputs, because degraded inputs are never seen.
Even with corruption augmentation, a learned gate risks overfitting to the specific corruption types seen during training.

The 0-parameter quality estimator sidesteps this entirely: spatial contrast is a physics-based proxy for image quality that generalizes across corruption types.
This is the same principle that makes the audio SNR estimator work: energy-based signal quality is corruption-agnostic.
...
NC-Conv adds minimal overhead: one extra depthwise convolution per block (static path) plus a 0-parameter quality computation.
The $\sim$246K parameter model fits comfortably on MCU-class hardware (\$3--5), enabling degradation-robust ADAS without GPU.
...
We presented NC-Conv, a noise-conditioned convolutional framework that blends dynamic (expressive) and static (robust) processing paths based on a zero-parameter quality estimator.
Transferring the NC theory from audio SSMs to vision CNNs, we achieve \textbf{+7.0\%} average improvement on corrupted CIFAR-10 at equal parameters, with up to \textbf{+19.1\%} on impulse noise.
The key insight is threefold: (1)~images are spatial, not sequential---CNN is the right backbone; (2)~video \emph{is} sequential along the temporal axis---SSM handles this naturally; (3)~the conditioning signal must be signal-processing-based, not learned, to generalize across degradation types.
NC-Conv establishes noise-conditioned processing as a unified, domain-agnostic framework spanning audio, images, and video.
