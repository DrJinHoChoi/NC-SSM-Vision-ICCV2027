...
% Blend equation in margin
\node[lbl, anchor=north, text=black!60] at (5.6, -3.8) {$\sigma h_d{+}(1{-}\sigma)h_s$};

\end{tikzpicture}%
}
\caption{\textbf{(a)} Standard CNN block: single fixed path. \textbf{(b)} NC-Conv block: a \textcolor{fixcolor}{static} path ($O(\sigma_n^2)$) and \textcolor{dyncolor}{dynamic} path ($O(\sigma_n^{4+})$) blended by learned $\sigma$ gate. Clean$\to\sigma{\approx}1$; degraded$\to\sigma{\approx}0$.}
\label{fig:ncconv}
\end{figure}
