...
All models are trained with per-sample multi-condition noise augmentation (Section~\ref{sec:training}) and evaluated on the GSC~V2 12-class test set under five noise types (factory, white, babble, street, pink) across SNR levels from $-15$ to $15$\,dB.

\subsection{Clean Accuracy and Parameter Efficiency}

Table~\ref{tab:clean} summarizes clean accuracy after noise-augmented training.
DS-CNN-S achieves the highest absolute accuracy (96.4\%) but requires 4.8$\times$ more parameters than NanoMamba-Tiny-DualPCEN.
When measured in accuracy per thousand parameters (\%/K), NanoMamba achieves \textbf{18.9\,\%/K}---4.6$\times$ more efficient than DS-CNN-S (4.1\,\%/K) and 1.5$\times$ more efficient than BC-ResNet-1 (12.8\,\%/K).

\begin{table}[t]
\centering
\caption{Clean accuracy and parameter efficiency on GSC V2 (12-class). All models trained with per-sample noise augmentation.}
\label{tab:clean}
\small
\begin{tabular}{lrrcc}
\toprule
\textbf{Model} & \textbf{Params} & \textbf{INT8} & \textbf{Acc.} & \textbf{\%/K} \\
\midrule
DS-CNN-S        & 23,756 & 23.2\,KB & 96.4\% & 4.1 \\
BC-ResNet-1     & 7,464  & 7.3\,KB  & 95.3\% & 12.8 \\
\midrule
NM-Tiny-DualPCEN & \textbf{4,957} & \textbf{4.8\,KB} & 93.7\% & \textbf{18.9} \\
\bottomrule
\end{tabular}
\end{table}
...
Table~\ref{tab:noise_main} reports accuracy under five noise types at representative SNR levels.
Fig.~\ref{fig:snr_curves} visualizes the full SNR--accuracy curves.

At moderate noise (0\,dB), NanoMamba-Tiny-DualPCEN achieves 85.1\% average accuracy across all noise types, compared to 91.2\% for DS-CNN-S (4.8$\times$ larger) and 88.1\% for BC-ResNet-1 (1.5$\times$ larger).
The accuracy gap is \emph{noise-type dependent}: babble noise shows the smallest gap ($-$4.8\,\%p vs DS-CNN-S), while broadband white and pink noise show the largest gaps ($-$6.3\,\%p and $-$5.7\,\%p).

This noise-type dependence is consistent with DualPCEN's design: babble noise has strong spectral structure (speech-like), enabling effective spectral-flatness-based routing, whereas broadband noise (white, pink) has flat spectra that challenge the SF/ST routing mechanism.

\begin{table*}[t]
\centering
\caption{Noise robustness comparison (accuracy \%). All models trained with per-sample noise augmentation. Best ultra-compact ($<$10K params) result per condition in \textbf{bold}.}
...
\multirow{3}{*}{Factory}
  ...
  & BC-ResNet-1 (7.5K)  & 95.3 & \textbf{64.5} & \textbf{74.8} & \textbf{83.5} & \textbf{89.0} & \textbf{91.9} & \textbf{92.9} & \textbf{93.9} \\
  & NM-DualPCEN (5.0K)  & 93.7 & 58.6 & 68.9 & 79.0 & 85.8 & 89.0 & 91.1 & 91.8 \\
\midrule
\multirow{3}{*}{White}
  & DS-CNN-S (23.8K)    & 96.8 & 61.7 & 67.1 & 81.6 & 89.9 & 93.1 & 94.8 & 95.6 \\
  & BC-ResNet-1 (7.5K)  & 95.3 & \textbf{58.6} & \textbf{64.6} & \textbf{76.7} & \textbf{85.9} & \textbf{90.2} & \textbf{92.6} & \textbf{93.8} \\
  & NM-DualPCEN (5.0K)  & 93.6 & 37.6 & 58.3 & 74.7 & 83.6 & 88.4 & 91.0 & 92.3 \\
\midrule
\multirow{3}{*}{Babble}
  ...
  & NM-DualPCEN (5.0K)  & 93.8 & 70.6 & 77.7 & 85.1 & 89.3 & 91.6 & 92.3 & 93.0 \\
\midrule
\multirow{3}{*}{Street}
  & DS-CNN-S (23.8K)    & 96.8 & 61.1 & 69.4 & 81.5 & 88.9 & 92.8 & 95.0 & 95.9 \\
  & BC-ResNet-1 (7.5K)  & 95.3 & \textbf{60.7} & \textbf{65.1} & \textbf{77.7} & \textbf{84.8} & \textbf{90.1} & \textbf{93.0} & \textbf{94.1} \\
  & NM-DualPCEN (5.0K)  & 93.8 & 58.2 & 64.6 & 71.7 & 83.3 & 87.3 & 91.0 & 92.4 \\
\midrule
\multirow{3}{*}{Pink}
  & DS-CNN-S (23.8K)    & 96.8 & 61.2 & 66.4 & 82.7 & 90.6 & 93.8 & 95.1 & 95.9 \\
  & BC-ResNet-1 (7.5K)  & 95.3 & \textbf{58.9} & \textbf{66.0} & \textbf{79.7} & \textbf{88.4} & \textbf{91.8} & \textbf{93.8} & \textbf{94.5} \\
  & NM-DualPCEN (5.0K)  & 93.8 & 28.3 & 56.7 & 75.4 & 84.8 & 89.4 & 91.9 & 93.2 \\
\bottomrule
\end{tabular}
\end{table*}
...
\begin{figure*}[t]
\centering
\includegraphics[width=\textwidth]{fig_snr_accuracy.png}
\caption{SNR vs.\ accuracy curves across five noise types. NanoMamba-Tiny-DualPCEN (5.0K params, solid blue) achieves competitive performance against CNN baselines with 1.5--4.8$\times$ more parameters. The gap narrows at higher SNR, with babble noise showing the smallest gap due to DualPCEN's spectral-flatness-based routing.}
\label{fig:snr_curves}
\end{figure*}


\subsection{Structural vs.\ Training-Based Robustness}
\label{subsec:clean_vs_noiseaug}

To disentangle \emph{structural} noise robustness (from architecture) and \emph{training-based} robustness (from noise-augmented data), we compare models trained on clean data only versus noise-augmented training, both evaluated at 0\,dB.
Table~\ref{tab:clean_vs_aug} and Fig.~\ref{fig:clean_vs_noiseaug} present this comparison.

\begin{table}[t]
\centering
\caption{Clean-only vs.\ noise-aug training at 0\,dB. ``$\Delta$'' shows noise-aug improvement over clean-only. NanoMamba's small $\Delta$ indicates high \emph{structural} robustness.}
...
\midrule
\multirow{3}{*}{\shortstack[l]{NM-DualPCEN\\(5.0K)}}
  & Clean  & 78.3 & 80.5 & 74.2 & 77.7 \\
  & NA     & 85.8 & 83.6 & 89.3 & 86.2 \\
  & $\Delta$ & \multicolumn{4}{c}{\textbf{+8.5\,\%p}} \\
\midrule
\multirow{3}{*}{\shortstack[l]{DS-CNN-S\\(23.8K)}}
  & Clean  & 75.6 & 13.9 & 70.1 & 53.2 \\
  & NA     & 91.5 & 89.9 & 94.5 & 92.0 \\
  & $\Delta$ & \multicolumn{4}{c}{+38.8\,\%p} \\
\midrule
\multirow{3}{*}{\shortstack[l]{BC-ResNet-1\\(7.5K)}}
...
\begin{figure}[t]
\centering
\includegraphics[width=\columnwidth]{fig_structural_summary.png}
\caption{Structural vs.\ training-based noise robustness.
NanoMamba retains 77.7\% accuracy at 0\,dB with clean-only training (structural robustness), requiring only +8.4\,\%p from noise-aug.
DS-CNN-S collapses to 53.2\% without noise-aug, requiring +38.7\,\%p---indicating near-total dependence on training-based robustness.}
\label{fig:clean_vs_noiseaug}
\end{figure}

The results reveal a striking asymmetry.
When trained on clean data only, NanoMamba-Tiny-DualPCEN retains \textbf{77.7\%} average accuracy at 0\,dB---24.5\,\%p higher than DS-CNN-S (\textbf{53.2\%}) despite having 4.8$\times$ fewer parameters.
DS-CNN-S catastrophically collapses under white noise to \textbf{13.9\%} (near random), while NanoMamba maintains \textbf{80.5\%}.
This demonstrates that NanoMamba's DualPCEN routing and SA-SSM adaptive dynamics provide substantial \emph{inherent} noise robustness without any noise during training.

Noise-augmented training reveals complementary effects: NanoMamba requires only +8.4\,\%p improvement from noise-aug (structural base is already strong), while DS-CNN-S requires +38.7\,\%p (almost entirely training-dependent).
This 4.6$\times$ ratio in noise-aug dependence directly quantifies the structural advantage of NanoMamba's architecture.


\subsection{Parameter Efficiency Analysis}

To enable fair comparison across models of different sizes, we analyze accuracy normalized by parameter count.
Table~\ref{tab:efficiency} shows that NanoMamba maintains a substantial efficiency advantage across all SNR conditions.

\begin{table}[t]
\centering
\caption{Parameter efficiency (\%/K-params) at representative SNR levels. Higher is better.}
\label{tab:efficiency}
\small
\begin{tabular}{lccc}
\toprule
\textbf{Model} & $\mathbf{-15}$\,\textbf{dB} & $\mathbf{0}$\,\textbf{dB} & \textbf{Clean} \\
\midrule
DS-CNN-S (23.8K)    & 2.8 & 3.8 & 4.1 \\
BC-ResNet-1 (7.5K)  & 8.6 & 11.8 & 12.8 \\
NM-DualPCEN (5.0K)  & \textbf{10.2} & \textbf{17.2} & \textbf{18.9} \\
\bottomrule
\end{tabular}
\end{table}

At 0\,dB, NanoMamba achieves 17.2\,\%/K---4.5$\times$ that of DS-CNN-S (3.8\,\%/K) and 1.5$\times$ that of BC-ResNet-1 (11.8\,\%/K).
Even at the extreme $-15$\,dB condition, NanoMamba maintains 10.2\,\%/K versus 2.8\,\%/K for DS-CNN-S (3.6$\times$ advantage).
This indicates that NanoMamba's structural mechanisms translate each parameter into more useful noise-robust computation than CNN architectures.


\subsection{Accuracy Drop Analysis}

Table~\ref{tab:drop} quantifies the accuracy degradation from clean to 0\,dB for each model--noise combination.
...
NM-DualPCEN & 8.5 & 10.1 & 4.4 & 11.6 & 8.8 \\
\bottomrule
\end{tabular}
\end{table}

NanoMamba shows the largest drop for street noise (11.6\,\%p) and the smallest for babble (4.4\,\%p).
The babble result is notable: the drop gap between NanoMamba and DS-CNN-S is only 1.8\,\%p, despite the 4.8$\times$ parameter difference.
Babble noise's speech-like spectral structure provides strong cues for DualPCEN's spectral flatness routing, allowing effective clean/noisy pathway separation.
Conversely, broadband noise (white, pink) provides weak spectral cues, challenging the SF/ST routing mechanism and resulting in larger drops.


\subsection{Ablation Study}

Table~\ref{tab:ablation} quantifies the contribution of each proposed component, evaluated at 0\,dB factory noise.

\begin{table}[t]
\centering
\caption{Ablation study (factory noise, 0\,dB). Each row adds one component.}
\label{tab:ablation}
\small
\begin{tabular}{lcc}
\toprule
\textbf{Configuration} & \textbf{Clean} & \textbf{0\,dB} \\
\midrule
NM-Tiny (base SA-SSM)          & 92.3 & 79.8 \\
\quad + DualPCEN                & 93.7 & 83.4 \\
\quad + Guarantees              & 93.7 & 84.1 \\
\quad + Noise-aug training      & 93.7 & 85.2 \\
\bottomrule
\end{tabular}
\end{table}

DualPCEN contributes the largest single improvement (+3.6\,\%p at 0\,dB), confirming the value of dual-expert adaptive normalization.
Architectural guarantees add +0.7\,\%p by preventing optimizer destruction of noise-robust properties, and noise-aug training contributes a further +1.1\,\%p.
Notably, DualPCEN also improves clean accuracy (+1.4\,\%p), indicating that dual-expert routing benefits general feature extraction beyond noise robustness.


\subsection{Runtime Calibration}

Runtime calibration adjusts the SA-SSM's $\Delta$-floor, $\varepsilon$-bypass, and $\mathbf{B}$-gate floor based on estimated noise level, requiring zero additional parameters.
This mechanism is exclusive to NanoMamba; CNN baselines have no equivalent runtime-adjustable noise parameters.
Calibration results will be reported upon completion of the ongoing evaluation
...
The noise-robustness results in Table~\ref{tab:noise_main} provide indirect evidence for routing effectiveness:
babble noise (speech-like spectral structure) shows the smallest performance gap versus CNN baselines ($-$4.8\,\%p at 0\,dB vs DS-CNN-S), while broadband noise (white: $-$6.3\,\%p, pink: $-$5.7\,\%p) shows larger gaps.
This is consistent with DualPCEN's spectral-flatness-based routing: structured noise activates Expert~1 (non-stationary, $\delta=2.0$), while broadband noise produces ambiguous SF values that weaken routing discrimination.
...
As shown in the ablation study (Table~\ref{tab:ablation}), adding guarantees contributes +0.7\,\%p at 0\,dB while maintaining clean accuracy.
This indicates that the optimizer, without constraints, partially destroys noise-robust dynamics during training.
...
The SNR-adaptive bypass mechanism (Section~\ref{sec:ss_bypass}) is designed to apply spectral subtraction only when beneficial: the bypass gate $\alpha$ approaches 1 at high SNR (preserving clean signals) and 0 at low SNR (enabling noise removal).
This mechanism prevents the well-known artifact introduction problem of spectral subtraction in clean or near-clean conditions~\cite{boll1979suppression,loizou2013speech}.
Evaluation with the spectral subtraction + bypass pipeline is ongoing and will quantify the additional noise robustness improvement, particularly at extreme SNR levels where the front-end enhancer is expected to provide the greatest benefit.
...
The clean-only vs.\ noise-aug comparison (Section~\ref{subsec:clean_vs_noiseaug}) provides the clearest evidence for the distinction between structural and training-based noise robustness.
NanoMamba retains 77.7\% at 0\,dB with clean-only training, while DS-CNN-S collapses to 53.2\% (white noise: 13.9\%)---a 24.5\,\%p structural advantage despite 4.8$\times$ fewer parameters.
The noise-aug dependence ratio (NanoMamba +8.4\,\%p vs DS-CNN-S +38.7\,\%p) directly quantifies this: CNN architectures are \emph{almost entirely dependent} on training data for noise robustness, while NanoMamba derives the majority from its architecture.

When all models are trained with noise augmentation, CNN baselines achieve higher absolute accuracy due to their larger parameter budgets.
However, this should not obscure the fundamental architectural difference: CNN's noise robustness is \emph{fragile} (disappears without noise-aug), while NanoMamba's is \emph{structural} (persists regardless of training data).

The noise-type-dependent gap provides further insight.
Babble noise shows the smallest gap ($-$4.8\,\%p vs DS-CNN-S at 0\,dB), confirming that DualPCEN's spectral-flatness routing is most effective when noise has distinctive spectral characteristics.
Broadband noise (white, pink) produces larger gaps, suggesting that future routing features (e.g., temporal modulation index) could further improve discrimination.

At equal parameter budgets ($\sim$7.5K), NanoMamba's structural advantages would be amplified, while CNN's fundamental limitation (frozen BatchNorm statistics at inference) remains unchanged regardless of scale.
...
The current evaluation uses a single dataset (GSC V2) with synthetically generated noise, and CNN baselines outperform NanoMamba in absolute accuracy due to their larger model capacity.
Several directions address these limitations:
(1)~\emph{Equal-parameter comparison}: scaling NanoMamba to $\sim$7.5K and $\sim$24K parameters to enable fair comparison at the same capacity;
(2)~\emph{Additional datasets}: GSC V1, multilingual KWS corpora (MLCommons MSWC~\cite{warden2018speech} for 50+ languages), and real-world noise recordings;
(3)~\emph{On-device profiling}: latency and energy measurements on ARM Cortex-M4/M7 and RISC-V;
(4)~\emph{Extended DualPCEN}: beyond two experts, including temporal-modulation routing for broadband noise;
(5)~\emph{KWS--LLM pipeline}: NanoMamba as an always-on gatekeeper for cloud LLM inference, exploiting the shared Mamba architecture family from edge (5K params) to cloud (billions).
...
With only \textbf{4,957~parameters}, NanoMamba-Tiny-DualPCEN achieves 93.7\% clean accuracy and retains \textbf{77.7\%} average accuracy at 0\,dB with clean-only training---24.5\,\%p higher than DS-CNN-S (53.2\%) despite having 4.8$\times$ fewer parameters.
DS-CNN-S collapses to 13.9\% under white noise without noise augmentation, while NanoMamba maintains 80.5\%.
This quantifies the \emph{structural robustness gap}: NanoMamba requires only +8.4\,\%p from noise-aug training, versus +38.7\,\%p for DS-CNN-S, demonstrating that CNN noise robustness is almost entirely training-dependent.

With noise-augmented training, NanoMamba delivers 4.6$\times$ higher parameter efficiency than DS-CNN-S, and DualPCEN contributes the largest single improvement (+3.6\,\%p at 0\,dB) with babble noise showing the smallest gap versus CNNs---confirming spectral-flatness-based routing effectiveness.

These results establish that ultra-compact SSMs with structural noise adaptation represent a principled approach to noise-robust edge KWS, uniquely suited for the most extreme memory-constrained deployments where CNN's training-dependent robustness is insufficient.
